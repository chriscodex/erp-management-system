...
import { deleteOrdenDeServicioRequestClient } from '@/app/taller/ordenes-servicio/[id]/_services/requests';
 ...
 */
export function DeleteOrdenDeServicioAlert({
  isOpen,
  setIsOpen,
  ordenDeServicioId,
  ...
  const handleConfirmationDeleteProduct = async () => {
    try {
      setIsOpen(false);
      toast.promise(deleteOrdenDeServicioRequestClient(ordenDeServicioId), {
        loading: 'Eliminando...',
        success: () => {
          ...
          if (actionAfterComplete === 'push') {
            router.push(`/taller/ordenes-servicio`);
            return `Orden de servicio eliminada correctamente`;
          }
        },
        error: (error) => {
      ...
      <AlertDialog open={isOpen} onOpenChange={setIsOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Esta acción no se puede deshacer. Esta orden de servicio será
